...
class _FocusPaletteState extends State<FocusPalette> {
  ...
  Widget build(BuildContext context) {
    ...
    // First appearance: bottom centre, clear of the notch on the Macs that
    // have one and out of the way of right-handed writing.
    final pos = _pos ??
        app.palettePos ??
        Offset(size.width / 2 - 210, size.height - 96);

    return Positioned(
      left: pos.dx.clamp(0.0, (size.width - 60).clamp(0.0, double.infinity)),
      top: pos.dy.clamp(0.0, (size.height - 60).clamp(0.0, double.infinity)),
      child: Material(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Padding(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                _divider(s),
                for (var i = 0; i < app.inkPalette.length; i++) _well(scheme, i),
                _divider(s),
                SizedBox(
                  width: 96,
                  child: Slider(
  ...
  /// panel is stuck.
  Widget _grip(BuildContext context, Size screen) => GestureDetector(
        behavior: HitTestBehavior.opaque,
        onPanUpdate: (d) => setState(() {
          final at = _pos ??
              app.palettePos ??
              Offset(screen.width / 2 - 210, screen.height - 96);
          _pos = at + d.delta;
        }),
